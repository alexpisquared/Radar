...
public class RadarPicCollector
{
  ...
  readonly SortedList<string, System.Drawing.Bitmap> _urlPicList = new(StringComparer.CurrentCultureIgnoreCase);
  ...
  void getPic(int times10minBack, DateTime gmt10Now, int period, string? rainOrSnow = null)
  {
    ...
    try
    {
      ...
      if (times10minBack < _backLenLive)
      {
        ...
        Pics.Add(new PicDetail(pic, dt.AddHours(-GmtOffset), _station[_stationIndex], _stationOffset[_stationIndex], WebScraper.GetCachedFileNameFromUrl_(url.Split('|')[0], false)));
      }
      else
      {
    ...
    try
    {
      ...
      for (int j = max - 1, i = 0; i < 72 && j >= 0; i++)
      {
        ...
        if (bmp != null)
        {
          ...
          //??mh[j] = PicMea.CalcMphInTheArea(bmp, DateTime.Now);
          mh[j] = PicMea.CalcMphInTheArea(bmp, i, j); //2024: i, j is wrong - just to compile.
          j--;
        }
      }
