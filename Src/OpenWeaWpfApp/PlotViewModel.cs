...
public partial class PlotViewModel : ObservableValidator
{
  ...
  void PopulateScatModel(object? obj)
  {
    ...
    _ = Task.Run(async () => { var dta = await PlotViewModelHelpers.GetFore24hrFromEC(Cnst._mississ); return dta; }).ContinueWith(_ => { DrawFore24hrEC(Cnst._mississ, _.Result); _foreMis = _.Result;         Model.Title += $"Miss \tat {(_foreMis?.currentConditions)?.dateTime[1].hour.Value}:{(_foreMis?.currentConditions)?.dateTime[1].minute}   {_foreMis?.currentConditions.temperature.Value,5:N1}°   {_foreMis?.currentConditions?.windChill?.Value ?? _foreMis?.currentConditions.temperature.Value,4:N0}° {_foreMis?.currentConditions?.wind?.speed?.Value,5:N1}{(_foreMis?.currentConditions?.wind)?.speed.units} \n";    }, TaskScheduler.FromCurrentSynchronizationContext());
    ...
    _ = Task.Run<object>(async () => await _opnwea.GetIt(_cfg["AppSecrets:MagicWeather"] ?? throw new ArgumentNullException(nameof(obj)), OpenWea.OpenWeatherCd.OneCallApi) ?? throw new ArgumentNullException(nameof(obj))).ContinueWith(async _ =>
    {
     ...
     Model.Title /*= CurrentConditions*/ += $"OWA \tat {OpenWea.UnixToDt(oca.current.dt):HH:mm}   {oca.current.temp,5:N1}°   {oca.current.feels_like,4:N0}° {oca.current.wind_speed * _ms2kh / _wk,5:N1}k/h                                                                                                                                                                                                        \n";
